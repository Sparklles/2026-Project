...
public class PopularBySalesStrategy implements RecommendStrategy {
    ...
    public List<ScoredBook> execute(Map<String, Object> params) {
        ...
        wrapper.orderByDesc(BookStats::getSales);
        ...
        for (BookStats stats : statsList) {
            ...
            if (valid) {
                result.add(new ScoredBook(stats.getBookId(),
                        BigDecimal.valueOf(stats.getSales() == null ? 0 : stats.getSales()), "畅销"));
            }
